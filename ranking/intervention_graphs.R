...
## This script contains the code for the 1) intervention boxplots, 2) the Hexbin plot, 
# and 3) the multidomain intervention boxplots


## Intervention Results Boxplots

cc_rank_cols = c("harmonic_rank", "harmonic_centrality", "pagerank_rank", "pagerank_centrality", "url", "a")
expbline <- read.csv('./output/ranks/exp-baseline.label_only.out', sep = '\t', col.names = cc_rank_cols, header = FALSE)
expbline <- as_tibble(expbline)
expbline$Intervention <- 'Pre-Intervention Baseline'
exppprrank <- read.csv('./output/ranks/exp-rel-ppr-ranks.label_only.txt', col.names = cc_rank_cols, sep = '\t', header = FALSE)
exppprrank <- as_tibble(exppprrank)
exppprrank$Intervention <- 'PPR on Reliable News'
explsppr <- read.csv('./output/ranks/exp-ls-ppr-down.label_only.out',col.names = cc_rank_cols, sep = '\t', header = FALSE)
explsppr <- as_tibble(explsppr) 
explsppr$Intervention <- 'Inv-PPR on Link Schemes'
expstrppr <- read.csv('./output/ranks/exp-ls-str-ppr-down.label_only.out',col.names = cc_rank_cols, sep = '\t', header = FALSE)
expstrppr <- as_tibble(expstrppr)
expstrppr$Intervention <- 'Inv-PPR on Anti-TrustRank Spam'
exppunrelprrank <- read.csv('./output/ranks/exp-unrel-ppr-down.label_only.out',col.names = cc_rank_cols, sep = '\t', header = FALSE)
exppunrelprrank <- as_tibble(exppunrelprrank)
exppunrelprrank$Intervention <- 'Inv-PPR on Unreliable News'
lsfr <-  read.csv('./output/ranks/ls_filtered-ranks.label_only.txt',col.names = cc_rank_cols, sep = '\t', header = FALSE)
lsfr <- as_tibble(lsfr)
lsfr$Intervention <- 'Edge Removal on Link Schemes'
lsafr <- read.csv('./output/ranks/ls_atr_filtered-ranks.label_only.txt.txt',col.names = cc_rank_cols, sep = '\t', header = FALSE)
...
df <- read.csv("output/exp-ls-atr-discover-top.out.sorted", sep = '\t', col.names = cc_rank_cols, header = FALSE)
df2 <- read.csv("output/exp-scam-casino-discover-top.out.sorted", sep = '\t', col.names = cc_rank_cols, header = FALSE)
...
ggsave('../fig/link_scheme_counts.png', dpi = 300, height = 11, width = 11, units = 'in')



### Multidomain Boxplots
...
eratmd <- read.csv("output/ls_multi_domain_atr/ls_multi_domain_atr-ranks.label_only.txt", sep = '\t', col.names = cc_rank_cols, header = FALSE)
eratmd <- as_tibble(eratmd)
eratmd$Intervention <- 'Edge Removal on Multi-Domain ATR'

eratnd <- read.csv("output/ls_combined_filtered/ls_combined_filtered-ranks.label_only.txt", sep = '\t', col.names = cc_rank_cols, header = FALSE)
...
ggplot(df, aes(x = label, y = log_pagerank_centrality, fill = Intervention)) +
  ...
  scale_x_discrete(labels=c("1" = "V. Unreliable", "3" = 'Unreliable', "4" = "Mixed",
                            "5" = "Reliable", "6" = 'V. Reliable'))
ggsave('../fig/interventions_boxplots_md.png', dpi = 300, height = 11, width = 16, units = 'in')
